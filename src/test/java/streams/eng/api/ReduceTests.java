...
public class ReduceTests {
    private static void testReduce() {
        ...
        // Return sum
        Integer sum = collection.stream().reduce((s1, s2) -> s1 + s2).orElse(0); // using sssst.stream Api
        ...
        // Return max
        Integer max1 = collection.stream().reduce((s1, s2) -> s1 > s2 ? s1 : s2).orElse(0); // using sssst.stream Api
        Integer max2 = collection.stream().reduce(Integer::max).orElse(0); // using sssst.stream Api using Integer::max
        ...
        // Return min
        Integer min = collection.stream().reduce((s1, s2) -> s1 < s2 ? s1 : s2).orElse(0);  // using sssst.stream Api
        ...
        // Return last element
        Integer last = collection.stream().reduce((s1, s2) -> s2).orElse(0); // using sssst.stream Api
        ...
        Integer sumMore2 = collection.stream().filter(o -> o > 2).reduce((s1, s2) -> s1 + s2).orElse(0);     // using sssst.stream Api
        ...
        Integer sumOdd = collection.stream().filter(o -> o % 2 != 0).reduce((s1, s2) -> s1 + s2).orElse(0); // using sssst.stream Api
